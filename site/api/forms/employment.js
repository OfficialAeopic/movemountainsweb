...
export default async function handler(req, res) {
  ...
  const required = ['first', 'last', 'email', 'phone', 'city', 'role', 'why'];
  ...
  const rows = [
    ...
    ['City', body.city],
    ['Role', body.role],
    ['Earliest start', body.start],
    ['Schedule type', body.schedule],
    ['Resume / LinkedIn', body.resume],
    ['Why this role', body.why],
    ['Relevant experience', body.experience],
    ['Available market weekends', checkbox(body.weekends)],
    ['Comfortable outdoors', checkbox(body.outdoors)],
    ['License and transportation', checkbox(body.license)],
    ['Notes', body.notes]
  ];

  return sendFormEmail(res, {
    subject: `New employment application: ${[body.first, body.last].filter(Boolean).join(' ')}`,
    replyTo: body.email,
    heading: 'New Employment Application',
    intro: 'Submitted from the Move Mountains employment application form.',
    rows
  });
}
...
async function sendFormEmail(res, { subject, replyTo, heading, intro, rows }) {
  ...
  try {
    ...
    console.error('[forms/employment] Resend error:', error);
